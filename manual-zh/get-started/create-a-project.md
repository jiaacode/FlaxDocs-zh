# 创建新项目

![Flax 项目页面](media/projects-page.jpg)

[获取 Flax](get-flax.md) 后，导航到 Flax Launcher 中的 **Projects** 页面。在这里你可以查看你的项目库。

你可以通过双击项目，使用默认引擎版本打开现有项目。使用项目名称右下角的三个点菜单，可以删除或取消链接项目，或者在文件资源管理器中打开项目文件的位置。

如果你是第一次使用 Flax，我们建议你点击 **Get Flax Samples** 下载一系列免费项目，这些项目展示了各种引擎功能，可以作为你使用 Flax 之旅的基础。Flax 示例可以在[此处](https://github.com/FlaxEngine/FlaxSamples)免费获取。

你也可以使用 **Add Project** 按钮添加现有项目。只需选择包含项目的文件夹，它就会出现在你的项目库中。

要创建新项目，请使用页面右上角的 **New Project** 按钮，并按照对话框的指示操作。你可以选择一个模板，并指定项目名称和目标位置。之后点击 **Create Project** 按钮，然后通过双击或使用项目库中的上下文菜单（三个点）打开它。

![新建 Flax 项目对话框](media/new-project-dialog.jpg)

现在是时候[了解 Flax 编辑器](editor.md)了……

### 从命令行创建项目

在不使用 Flax Launcher 的情况下创建新项目的另一种方法是运行 Flax Editor 可执行文件，并带上 `-new -project <new_project_path>` 参数，以便在指定的文件夹内创建新项目。

<div class="frontpage">

<div class="frontpage-section">
<a href="editor.md"><img src="../editor/media/icon.jpg"></a>
<h3><a href="editor.md">编辑器</a></h3>
<p>学习如何使用 Flax 编辑器。</p>
</div>

</div>

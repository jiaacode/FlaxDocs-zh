# 后期处理效果

![后期处理效果](/manual/media/postFx.png)

**后期处理效果**（或称 **PostFx**）通常应用于渲染后的场景图像，但在 UI 绘制之前。
你可以使用后期处理效果来确立场景的外观，并为它们增添更多真实感。

## 本节内容

* [环境光遮蔽](ambient-occlusion.md)
* [抗锯齿](anti-aliasing.md)
* [泛光](bloom.md)
* [摄像机伪影](camera-artifacts.md)
* [颜色分级](color-grading.md)
* [景深](depth-of-field.md)
* [运动模糊](motion-blur.md)
* [眼睛适应](eye-adaptation.md)
* [镜头光晕](lens-flares.md)
* [屏幕空间反射](screen-space-reflections.md)
* [色调映射](tone-mapping.md)
* [后期处理效果体积](post-fx-volumes.md)
* [后期处理材质](post-fx-materials.md)

## 使用后期处理效果

如下图所示，使用后期处理效果可以极大地改变游戏的外观。

![后期处理效果](/manual/media/postFx-compare.png)

# 人工智能

![人工智能](behavior-trees/media/bt-debug.gif)

游戏中的**人工智能**（缩写为 **AI**）通常指创建可能看起来具有部分智能的角色或对象。通常，这种效果是通过组合多个不同的系统来实现的，例如：[行为树](behavior-trees/index.md)、[寻路](../../navigation/index.md)、[射线投射](../../physics/raycasting.md) 和游戏逻辑代码。

## 本节内容

* [行为树](behavior-trees/index.md)
  * [行为知识](behavior-trees/knowledge.md)
  * [行为](behavior-trees/behavior.md)
  * [行为树节点](behavior-trees/nodes.md)
  * [行为树装饰器](behavior-trees/decorators.md)
  * [如何创建自定义行为树节点](behavior-trees/custom-node.md)
  * [如何创建自定义行为树装饰器](behavior-trees/custom-decorator.md)
  * [如何创建自定义 Move To 节点](behavior-trees/custom-move-to.md)

## 游戏 AI 基础视频介绍

<center>
<iframe width="750" height="421" src="https://www.youtube.com/embed/G5A0-_4dFLg?si=BugIfCl1W3B-e7WM" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
</center>
